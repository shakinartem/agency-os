...
def record(rubric: str, *, views: int = 100, clicks: int = 0, leads: int = 0, conversions: int = 0, content_type: str = "post", platform: str = "telegram", account: str = "a"):
    return {
        "rubric": rubric,
...
def test_learning_refuses_to_optimize_tiny_samples():
    result = derive_learning_context([record("Lucky", clicks=50)], primary_metric="ctr", min_action_sample=5)
...
def test_business_metric_is_explicit_and_stable():
    ...
    views = derive_learning_context(rows, primary_metric="views_per_publication", min_action_sample=5)
    conversions = derive_learning_context(rows, primary_metric="conversions_per_1000_views", min_action_sample=5)
